...
curl -v $URL -H "Authorization: Basic YWRtaW4wMDEq" -d "statement=select count(*) from models" | python -mjson.tool > $DIR/count-star.json
curl -v $URL -H "Authorization: Basic YWRtaW4wMDEq" -d "statement=select count(*) from models where _type='Portfolio'" | python -mjson.tool > $DIR/portfolios.json
curl -v $URL -H "Authorization: Basic YWRtaW4wMDEq" -d "statement=select * from system:indexes" | python -mjson.tool > $DIR/indexes.json
