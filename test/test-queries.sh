...
curl -v $URL -d "saslPassword=$ADMIN_PASSWORD" -d "statement=select count(*) from models" | python -mjson.tool > $DIR/count-star.json
curl -v $URL -d "saslPassword=$ADMIN_PASSWORD" -d "statement=select count(*) from models where _type='Portfolio'" | python -mjson.tool > $DIR/portfolios.json
curl -v $URL -d "saslPassword=$ADMIN_PASSWORD" -d "statement=select * from system:indexes" | python -mjson.tool > $DIR/indexes.json
